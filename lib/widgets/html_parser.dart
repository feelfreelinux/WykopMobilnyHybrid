...
class HtmlWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        ...
        child: (new _HtmlParser(context)).parseFromStr(strippedHtml));
  }
}
  ...
  void _parseElement(html.Element element) {
    ...
    switch (element.localName) {
      ...
      case 'a':
        ...
        return;
      case 'br':
        // _appendToCurrentTextSpans(TextSpan(text: '\n'));
        //       _tryCloseCurrentTextSpan();
        for (var subNode in element.nodes) {
          _parseNode(subNode);
        }
...
class HtmlRichTextParser extends StatelessWidget {
  ...
  void _parseNode(dom.Node node, ParseContext parseContext) {
    ...
    if (node is dom.Text && parseContext.parentElement != 'dupa') {
      ...
      if (finalText.contains('[pokaż spoiler]')) {
        return;
      }
        ...
        switch (node.localName) {
          case "a":
            ...
            if (url.startsWith('spoiler:')) {
              var linkContainer = new SpoilerWidget(node.attributes['href']);
...
class SpoilerWidget extends StatelessWidget {
  ...
  SpoilerWidget(this.spoilerValue);

  String _getSpoiler() {
    return Uri.decodeFull(spoilerValue.replaceFirst('spoiler:', '').replaceAll('+', ' '));
  }
  @override
  Widget build(BuildContext context) {
